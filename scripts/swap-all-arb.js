// Swap 100 ARB → USDC via Uniswap V3 (0.05% pool, batches of 20)
require("dotenv").config();
const { ethers } = require("ethers");

async function main() {
  const p = new ethers.JsonRpcProvider("https://arb-mainnet.g.alchemy.com/v2/QcDRXt-r9WOv3CjtZ0WE0");
  const w = new ethers.Wallet(process.env.DEPLOYER_PRIVATE_KEY, p);
  const router = new ethers.Contract("0x68b3465833fb72A70ecDF485E0e4C7bD8665Fc45", [
    "function exactInputSingle((address tokenIn, address tokenOut, uint24 fee, address recipient, uint256 amountIn, uint256 amountOutMinimum, uint160 sqrtPriceLimitX96)) external payable returns (uint256 amountOut)"
  ], w);
  const arb = new ethers.Contract("0x912CE59144191C1204E64559FE8253a0e49E6548", [
    "function balanceOf(address) view returns (uint256)",
    "function approve(address,uint256) returns (bool)",
    "function allowance(address,address) view returns (uint256)",
  ], w);
  const usdc = new ethers.Contract("0xaf88d065e77c8cC2239327C5EDb3A432268e5831", ["function balanceOf(address) view returns (uint256)"], p);

  console.log("ARB:", ethers.formatEther(await arb.balanceOf(w.address)));
  console.log("USDC:", ethers.formatUnits(await usdc.balanceOf(w.address), 6));

  // Ensure approval
  const allowance = await arb.allowance(w.address, "0x68b3465833fb72A70ecDF485E0e4C7bD8665Fc45");
  if (allowance < ethers.parseEther("200")) {
    console.log("Approving ARB...");
    const atx = await arb.approve("0x68b3465833fb72A70ecDF485E0e4C7bD8665Fc45", ethers.MaxUint256);
    await atx.wait();
    console.log("Approved ✓");
  }

  // Swap in batches of 20 ARB to avoid liquidity issues
  const batches = [20, 20, 20, 20, 20];
  for (let i = 0; i < batches.length; i++) {
    const amount = ethers.parseEther(String(batches[i]));
    const currentArb = await arb.balanceOf(w.address);
    if (currentArb < amount) {
      console.log(`\nSkipping batch ${i+1} — only ${ethers.formatEther(currentArb)} ARB left`);
      break;
    }

    console.log(`\n--- Batch ${i+1}: Swapping ${batches[i]} ARB → USDC ---`);
    
    // Try 0.05% first, then 0.3%
    for (const fee of [500, 3000]) {
      try {
        const tx = await router.exactInputSingle({
          tokenIn: "0x912CE59144191C1204E64559FE8253a0e49E6548",
          tokenOut: "0xaf88d065e77c8cC2239327C5EDb3A432268e5831",
          fee,
          recipient: w.address,
          amountIn: amount,
          amountOutMinimum: 0,
          sqrtPriceLimitX96: 0,
        }, { gasLimit: 300000 });
        console.log("Tx:", tx.hash);
        await tx.wait();
        console.log("✓ Done");
        break; // success, move to next batch
      } catch (e) {
        console.log(`Fee ${fee} failed, trying next...`);
      }
    }
  }

  // Final
  console.log("\n========== FINAL BALANCES ==========");
  console.log("ETH:", ethers.formatEther(await p.getBalance(w.address)));
  console.log("ARB:", ethers.formatEther(await arb.balanceOf(w.address)));
  console.log("USDC:", ethers.formatUnits(await usdc.balanceOf(w.address), 6));
}

main().catch(e => console.error("Error:", e.message));
