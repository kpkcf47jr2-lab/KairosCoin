...
¿Qué necesitas?`)}async _analyze(e,s="1h",o=200){const i=`${e}_${s}`,p=this._cache.get(i);if(p&&Date.now()-p.ts<this._cacheTTL)return p.data;const[a,r]=await Promise.all([H.getCandles(e,s,o),H.get24hrTicker(e)]);if(!a||a.length<50)throw new Error(`Datos insuficientes para ${e}`);const t=a.map(u=>u.close),c=a.map(u=>u.high),m=a.map(u=>u.low),d=a.map(u=>u.volume),n=t.length,b=N(t,20),g=N(t,50),f=Y(t,200),l=ee(t,14),S=ae(t,12,26,9),v=oe(t,20,2),$=se(a),A=t[n-1],M=b[n-1],E=g[n-1],y=f[n-1],C=l[n-1],U=S.macd[n-1],G=S.signal[n-1],D=S.histogram[n-1],B=v.upper[n-1],I=v.lower[n-1],z=v.middle[n-1],k=$[n-1],_=A>M?"alcista":"bajista",L=A>E?"alcista":"bajista",j=y?A>y?"alcista":"bajista":"indefinida",q=M>E&&(y?E>y:!0),T=W(b,g,n-1),P=W(S.macd,S.signal,n-1),w=ie(t,l,14),x=B&&I?(B-I)/z*100:0,R=[];for(let u=Math.max(0,n-20);u<n;u++)v.upper[u]&&v.lower[u]&&v.middle[u]&&R.push((v.upper[u]-v.lower[u])/v.middle[u]*100);const J=R.length>0?R.reduce((u,F)=>u+F,0)/R.length:x,Q=x<J*.75,V=d.slice(-20).reduce((u,F)=>u+F,0)/20,O=d[n-1]/V,X=Math.max(...c.slice(-20)),Z=Math.min(...m.slice(-20));let h=50;_==="alcista"&&(h+=8),L==="alcista"&&(h+=8),j==="alcista"&&(h+=8),q&&(h+=6),C>50&&C<70&&(h+=5),C<30&&(h+=10),C>70&&(h-=10),D>0&&(h+=5),P==="bullish_cross"&&(h+=10),P==="bearish_cross"&&(h-=10),T==="bullish_cross"&&(h+=10),T==="bearish_cross"&&(h-=10),A>k&&(h+=5),w==="bullish_divergence"&&(h+=8),w==="bearish_divergence"&&(h-=8),O>1.5&&(h+=_==="alcista"?5:-5),h=Math.max(0,Math.min(100,h));const K={pair:e,price:A,ticker:r,ind:{ema20:M,ema50:E,sma200:y,rsi:C,macdLine:U,macdSig:G,macdHist:D,bbU:B,bbL:I,bbM:z,bbWidth:x,squeeze:Q,vwap:k},trend:{short:_,mid:L,long:j,aligned:q,emaCross:T},signals:{rsiDiv:w,macdCross:P,volRatio:O,avgVol:V},levels:{support:Z,resistance:X},score:h,candles:a};return this._cache.set(i,{data:K,ts:Date.now()}),K}async _fullAnalysis(e,s,o=null){var i,p;try{const a=await this._analyze(e),{price:r,ind:t,trend:c,signals:m,levels:d,score:n,ticker:b}=a;let g,f;n>=70?(g="FUERTEMENTE ALCISTA",f="🟢🟢"):n>=55?(g="MODERADAMENTE ALCISTA",f="🟢"):n>=45?(g="NEUTRAL / LATERAL",f="🟡"):n>=30?(g="MODERADAMENTE BAJISTA",f="🔴"):(g="FUERTEMENTE BAJISTA",f="🔴🔴");const l=[];c.emaCross==="bullish_cross"&&l.push("✅ **Golden Cross** — EMA20 cruzó encima de EMA50"),c.emaCross==="bearish_cross"&&l.push("❌ **Death Cross** — EMA20 cruzó debajo de EMA50"),m.macdCross==="bullish_cross"&&l.push("✅ **MACD Bullish Cross**"),m.macdCross==="bearish_cross"&&l.push("❌ **MACD Bearish Cross**"),t.rsi<30&&l.push("⚡ **RSI Sobreventa** — Posible rebote"),t.rsi>70&&l.push("⚠️ **RSI Sobrecompra** — Posible corrección"),m.rsiDiv==="bullish_divergence"&&l.push("🔥 **Divergencia RSI Alcista** — Reversal probable"),m.rsiDiv==="bearish_divergence"&&l.push("⚠️ **Divergencia RSI Bajista**"),t.squeeze&&l.push("💥 **BB Squeeze** — Explosión inminente"),m.volRatio>2&&l.push(`📊 **Volumen ${m.volRatio.toFixed(1)}x** sobre promedio`),c.aligned&&l.push("📈 **EMAs Alineadas** — Tendencia fuerte");let S;n>=65?S=`🟢 **BUSCAR COMPRAS** en pullback a EMA20 ($${t.ema20.toFixed(2)}). SL debajo EMA50. TP: $${d.resistance.toFixed(2)}.`:n>=45?S=`🟡 **ESPERAR** — Operar rango $${d.support.toFixed(2)} - $${d.resistance.toFixed(2)}.`:S=`🔴 **PRECAUCIÓN** — Evitar longs. Short target: $${d.support.toFixed(2)}.`;const v=(b==null?void 0:b.changePercent)||0,$=`📊 **ANÁLISIS — ${e}**
...
💡 _Di "crea un script para ${e}" para generar código automático._`;return this._reply($,null,null,o)}catch(a){if(s)return this._basicAnalysis(s);throw a}}_quickSummary(e,s){var t;const{price:o,ind:i,trend:p,score:a}=s;return`${a>=60?"🟢":a>=40?"🟡":"🔴"} **${e}** — $${o.toLocaleString(void 0,{minimumFractionDigits:2})}
...
Resistencia: $${a.levels.resistance.toFixed(2)}`}catch{}const p="Eres Kairos AI, experto de trading de Kairos 777 (Kairos 777 Inc). Responde en español. Sé directo y profesional. Usa datos reales proporcionados. Nunca des consejos financieros personales. Incluye siempre SL/TP en estrategias."+(o.kairosScript?`
...
//# sourceMappingURL=ai-btJnoXEe.js.map
